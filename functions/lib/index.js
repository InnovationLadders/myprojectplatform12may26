...
const btn = (href, label, color = "#0ea5e9") => `
...
function buildHtml(p) {
    var _a;
    const appUrl = ((_a = p.login_url) === null || _a === void 0 ? void 0 : _a.split("/").slice(0, 3).join("/")) || "https://mashroui.com";
    switch (p.type) {
        case "admin_new_student": {
            return {
                subject: `طلب انضمام طالب جديد - ${p.student_name || ""}`,
                html: wrap("linear-gradient(135deg,#0ea5e9 0%,#0284c7 100%)", "طلب انضمام طالب جديد", `<p style="font-size:17px;color:#1f2937;margin:0 0 16px;">مرحباً ${p.to_name}،</p>
           ...
           ${infoBox("#f0f9ff", "#bae6fd", `<p style="margin:0 0 8px;color:#0369a1;font-weight:600;font-size:15px;">بيانات الطالب</p>
        ...
        case "student_activated": {
            return {
                ...
                html: wrap("linear-gradient(135deg,#10b981 0%,#059669 100%)", "تمت الموافقة على حسابك", `<p style="font-size:17px;color:#1f2937;margin:0 0 16px;">مرحباً ${p.to_name}،</p>
           ...
           ${infoBox("#f0fdf4", "#bbf7d0", `<p style="margin:0;color:#166534;font-size:14px;">إذا واجهت أي مشكلة عند تسجيل الدخول يرجى التواصل مع إدارة مؤسستك أو فريق الدعم.</p>`)}
           ${btn(p.login_url || `${appUrl}/login`, "تسجيل الدخول الآن", "#10b981")}`),
            };
        }
        case "project_created": {
            const membersHtml = (p.team_members || []).map(m => `<p style="margin:4px 0;color:#1e3a5f;">• <strong>${m.name}</strong> — ${m.role === "leader" ? "قائد الفريق" : "عضو"}</p>`).join("");
            return {
                subject: `تمت إضافتك إلى مشروع: ${p.project_title || ""}`,
                html: wrap("linear-gradient(135deg,#10b981 0%,#059669 100%)", "مشروع جديد", `<p style="font-size:17px;color:#1f2937;margin:0 0 16px;">مرحباً ${p.to_name}،</p>
           <p style="color:#4b5563;line-height:1.7;margin:0 0 24px;">تمت إضافتك إلى مشروع جديد في منصة مشروعي.</p>
           ${infoBox("#f0fdf4", "#bbf7d0", `<p style="margin:0 0 8px;color:#166534;font-weight:600;font-size:15px;">${p.project_title || ""}</p>
        ...
        case "consultation_requested": {
            return {
                ...
                html: wrap("linear-gradient(135deg,#8b5cf6 0%,#6d28d9 100%)", "طلب استشارة جديد", `<p style="font-size:17px;color:#1f2937;margin:0 0 16px;">مرحباً ${p.to_name}،</p>
           ...
           ${infoBox("#f5f3ff", "#ddd6fe", `<p style="margin:0 0 8px;color:#5b21b6;font-weight:600;font-size:15px;">تفاصيل الطلب</p>
        ...
        case "consultation_accepted": {
            return {
                ...
                html: wrap("linear-gradient(135deg,#0ea5e9 0%,#0284c7 100%)", "تم قبول طلب الاستشارة", `<p style="font-size:17px;color:#1f2937;margin:0 0 16px;">مرحباً ${p.to_name}،</p>
           ...
           ${infoBox("#f0f9ff", "#bae6fd", `<p style="margin:0 0 8px;color:#0369a1;font-weight:600;font-size:15px;">تفاصيل الاستشارة</p>
              <p style="margin:4px 0;color:#1e3a5f;"><strong>الموضوع:</strong> ${p.consultation_topic || ""}</p>
        ...
        case "consultation_completed": {
            return {
                ...
                html: wrap("linear-gradient(135deg,#10b981 0%,#059669 100%)", "تم إكمال الاستشارة", `<p style="font-size:17px;color:#1f2937;margin:0 0 16px;">مرحباً ${p.to_name}،</p>
           ...
           ${infoBox("#f0fdf4", "#bbf7d0", `<p style="margin:0;color:#166534;font-size:14px;">نرجو منك تقييم الاستشارة لمساعدتنا على تحسين الخدمة.</p>`)}
        ...
        case "investor_request_new": {
            return {
                ...
                html: wrap("linear-gradient(135deg,#f59e0b 0%,#d97706 100%)", "طلب اهتمام مستثمر جديد", `<p style="font-size:17px;color:#1f2937;margin:0 0 16px;">مرحباً ${p.to_name}،</p>
           ...
           ${infoBox("#fffbeb", "#fde68a", `<p style="margin:0 0 8px;color:#92400e;font-weight:600;font-size:15px;">تفاصيل الطلب</p>
        ...
        case "investor_request_status_update": {
            ...
            return {
                ...
                html: wrap("linear-gradient(135deg,#0ea5e9 0%,#0284c7 100%)", "تحديث حالة طلب الاستثمار", `<p style="font-size:17px;color:#1f2937;margin:0 0 16px;">مرحباً ${p.to_name}،</p>
           <p style="color:#4b5563;line-height:1.7;margin:0 0 24px;">تم تحديث حالة طلبك للمشروع <strong>${p.project_title || ""}</strong>.</p>
           ${infoBox("#f0f9ff", "#bae6fd", `<p style="margin:0 0 8px;color:#0369a1;font-weight:600;font-size:15px;">الحالة الجديدة: ${statusLabel}</p>
        ...
        case "achievement_unlocked": {
            ...
            return {
                ...
                html: wrap("linear-gradient(135deg,#f59e0b 0%,#d97706 100%)", `وسام ${medalName}`, `<p style="font-size:17px;color:#1f2937;margin:0 0 16px;">مبروك ${p.to_name}!</p>
           ...
           ${infoBox("#fffbeb", "#fde68a", `<p style="margin:0 0 4px;color:#92400e;font-weight:600;">النقاط المكتسبة عند الفتح: ${p.achievement_points || 0}</p>
        ...
        case "first_daily_chat": {
            return {
                ...
                html: wrap("linear-gradient(135deg,#3b82f6 0%,#2563eb 100%)", "رسالة جديدة في المشروع", `<p style="font-size:17px;color:#1f2937;margin:0 0 16px;">مرحباً ${p.to_name}،</p>
           <p style="color:#4b5563;line-height:1.7;margin:0 0 24px;">لديك نشاط جديد في مشروعك اليوم.</p>
           ${infoBox("#eff6ff", "#bfdbfe", `<p style="margin:0 0 8px;color:#1e40af;font-weight:600;">${p.project_title || ""}</p>
...
exports.deleteUserAccount = functions
    ...
    var _a, _b;
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to delete a user account.");
    }
    const callerUid = context.auth.uid;
    const targetUid = data === null || data === void 0 ? void 0 : data.userId;
    if (!targetUid) {
    ...
    try {
        ...
        if (callerDoc.exists) {
            callerRole = ((_a = callerDoc.data()) === null || _a === void 0 ? void 0 : _a.role) || "";
        }
    }
    catch (err) {
    ...
    try {
        ...
        if (((_b = err === null || err === void 0 ? void 0 : err.errorInfo) === null || _b === void 0 ? void 0 : _b.code) === "auth/user-not-found") {
            ...
            throw new functions.https.HttpsError("internal", "Failed to delete authentication account: " + ((err === null || err === void 0 ? void 0 : err.message) || "unknown error"));
        }
    }
...
exports.bulkDeleteUserAccounts = functions
    .https.onCall(async (data, context) => {
    var _a, _b;
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to delete user accounts.");
    }
    const callerUid = context.auth.uid;
    const userIds = data === null || data === void 0 ? void 0 : data.userIds;
    if (!userIds || !Array.isArray(userIds) || userIds.length === 0) {
        throw new functions.https.HttpsError("invalid-argument", "Missing or invalid field: userIds");
    }
    ...
    for (const targetUid of userIds) {
        ...
        try {
            ...
            results.failed.push({
                id: targetUid,
                error: (err === null || err === void 0 ? void 0 : err.message) || "حدث خطأ غير معروف",
            });
        }
    }
    return results;
