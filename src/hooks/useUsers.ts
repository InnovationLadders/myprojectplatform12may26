    ...
    try {
      ...
      const usersData = await Promise.all(userDocsToFetch.map(async (userDoc) => {
        ...
        const processedUser = {
          ...
          location: userData.location || userData.city || 'غير محدد',
    ...
    try {
      ...
      console.log('📤 Saving user profile to Firestore:', {
        ...
        profileData: {
          ...
          hourly_rate_type: typeof userProfile.hourly_rate,
  ...
  const editUser = async (userId: string, userData: any) => {
    try {
      ...
      if (userData.password && userData.password.trim() !== '') {
        ...
        console.warn('⚠️ Password update for other users requires Firebase Admin SDK.');
        console.log('💡 Recommendation: Use "Send Password Reset Email" feature instead');

        // For now, we'll throw an informative error
        throw new Error('تحديث كلمة المرور لمستخدم آخر يتطلب إرسال رابط إعادة تعيين كلمة المرور عبر البريد الإلكتروني. يُنصح باستخدام خاصية "إرسال رابط إعادة تعيين كلمة المرور" بدلاً من ذلك.');
      }
