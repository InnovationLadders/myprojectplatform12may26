  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        ...
        // Limit to last 10 notifications for dashboard
    ...
    const getChatNotifications = async (projectIds: string[]): Promise<Notification[]> => {
      ...
      try {
        ...
        for (const projectId of projectIds.slice(0, 5)) { // Limit to 5 projects for performance
          ...
          const sortedMessages = messagesSnapshot.docs
            ...
            .slice(0, 2); // Limit to 2 recent messages per project
    ...
    const getOverdueProjectNotifications = async (): Promise<Notification[]> => {
      ...
      try {
        ...
        // Limit to first 5 projects to reduce load
        for (const projectId of userProjectIds.slice(0, 5)) {
          const projectDoc = await getDoc(doc(db, 'projects', projectId));
          if (projectDoc.exists()) {
    ...
    // Removed real-time listeners to improve performance
    // Dashboard will refresh notifications on each load
    // For real-time updates, consider implementing a polling mechanism with longer intervals
