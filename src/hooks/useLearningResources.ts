...
export const useLearningResources = () => {
  ...
  const createResource = async (resourceData: Partial<LearningResource>) => {
    try {
      // Validate video URLs before creating
      if (resourceData.type === 'video' && resourceData.videoUrl) {
        const validation = validateVideoUrls(resourceData.videoUrl);
        if (validation.isMock) {
          throw new Error('لا يمكن إنشاء فيديو برابط وهمي. الرجاء استخدام رابط فيديو حقيقي من YouTube');
        }
        if (!validation.isValid) {
          throw new Error(validation.error || 'رابط الفيديو غير صالح');
        }
      }
      ...
      const newResource = {
        ...
        embed_url: embedUrl || resourceData.embedUrl || null,
  ...
  const updateResource = async (id: string, resourceData: Partial<LearningResource>) => {
    try {
      // Validate all video URL fields before updating
      if (resourceData.type === 'video') {
        const urlFields = [
          { field: 'videoUrl', value: resourceData.videoUrl },
          { field: 'watchUrl', value: resourceData.watchUrl },
          { field: 'embedUrl', value: resourceData.embedUrl },
          { field: 'contentUrl', value: resourceData.contentUrl }
        ];

        for (const urlField of urlFields) {
          if (urlField.value && isMockVideoUrl(urlField.value)) {
            throw new Error(`لا يمكن تحديث الفيديو برابط وهمي في حقل ${urlField.field}. الرجاء استخدام رابط فيديو حقيقي من YouTube`);
          }
        }
      ...
      // Generate all video URLs for consistency
      let watchUrl = resourceData.watchUrl;
      let embedUrl = resourceData.embedUrl;
      let contentUrl = resourceData.contentUrl;

      if (resourceData.type === 'video' && resourceData.videoUrl) {
        const generatedUrls = generateAllVideoUrls(resourceData.videoUrl);
        if (generatedUrls) {
      ...
      if (embedUrl !== undefined) updateData.video_url = embedUrl;
