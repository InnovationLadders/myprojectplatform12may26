...
// Temporary interface that supports both old (string) and new (MultiLang) formats
...
export interface ProjectIdea {
  ...
  category: string;
  difficulty: string;
  duration: string | MultiLangText;
  subject: string | MultiLangText;
  image: string;
  objectives: string[] | MultiLangArray;
  materials: string[] | MultiLangArray;
  steps: string[] | MultiLangArray;
  tags: string[] | MultiLangArray;
  views: number;
  downloads: number;
  rating: number;
...
export const useProjectIdeas = () => {
  ...
  const fetchProjectIdeas = async () => {
    try {
      ...
      const ideas = isAdmin
        ? await getAllProjectIdeas(currentLanguage)
        : await getProjectIdeas(currentLanguage);

      console.log("Project ideas fetched:", ideas?.length || 0, 'ideas:', ideas);
  ...
  }, [user?.id, user?.role, currentLanguage]);
  ...
  return {
    ...
    getProjectIdeaById: async (id: string) => {
      try {
        const idea = await getProjectIdeaById(id, currentLanguage);
        return idea;
      } catch (err) {
