...
console.log('📦 ClasseraIntegration: Module loaded successfully');
...
export const ClasseraIntegration: React.FC = () => {
  console.log('🎯 ClasseraIntegration: Component initializing...');
  ...
  // Load sync batches from database
  useEffect(() => {
    console.log('📂 ClasseraIntegration: useEffect triggered - loading sync batches...');
    const loadSyncBatches = async () => {
      try {
        console.log('⏳ ClasseraIntegration: Starting sync batch load...');
        const startTime = Date.now();
        setLoading(true);
        // TODO: Implement actual sync batch loading from database
        // For now, just set empty array until real implementation
        setSyncBatches([]);
        const loadTime = Date.now() - startTime;
        console.log(`✅ ClasseraIntegration: Sync batches loaded successfully in ${loadTime}ms`);
        console.log('📊 ClasseraIntegration: Sync batches count:', 0);
        setLoading(false);
      } catch (err) {
        console.error('❌ ClasseraIntegration: Error loading sync batches:', err);
        console.error('🚫 ClasseraIntegration: Error details:', {
          message: err instanceof Error ? err.message : 'Unknown error',
          stack: err instanceof Error ? err.stack : undefined
        });
        setError('Failed to load sync batches');
        setLoading(false);
      }
    };
  ...
  console.log('⚙️ ClasseraIntegration: Generating LTI configuration...');
  const ltiConfig = generateLTIToolConfiguration();
  console.log('✅ ClasseraIntegration: LTI config generated:', ltiConfig);

  console.log('🔐 ClasseraIntegration: Generating JWKS...');
  const jwks = generateJWKS();
  console.log('✅ ClasseraIntegration: JWKS generated');

  const copyToClipboard = async (text: string, field: string) => {
    console.log('📋 ClasseraIntegration: Attempting to copy field:', field);
    try {
      await navigator.clipboard.writeText(text);
      console.log('✅ ClasseraIntegration: Successfully copied to clipboard:', field);
      setCopiedField(field);
      setTimeout(() => setCopiedField(null), 2000);
    } catch (err) {
      console.error('❌ ClasseraIntegration: Failed to copy:', err);
    }
  };
  ...
  const handleRefresh = async () => {
    console.log('🔄 ClasseraIntegration: Refresh button clicked');
    const startTime = Date.now();
    setIsRefreshing(true);
    // Simulate API call
    setTimeout(() => {
      const refreshTime = Date.now() - startTime;
      console.log(`✅ ClasseraIntegration: Refresh completed in ${refreshTime}ms`);
      setIsRefreshing(false);
    }, 1000);
  };

  const triggerManualSync = async () => {
    console.log('🔄 ClasseraIntegration: Manual sync button clicked');
    console.log('🛠️ ClasseraIntegration: Triggering manual sync with Classera...');
    // Implement manual sync logic
  };

  if (loading) {
    console.log('⏳ ClasseraIntegration: Rendering loading state...');
    return (
  ...
  console.log('🎨 ClasseraIntegration: Rendering main content');
  console.log('📊 ClasseraIntegration: Current state:', {
    syncBatchesCount: syncBatches.length,
    loading,
    error,
    isRefreshing,
    showLTIConfig,
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => {
                console.log('🖱️ ClasseraIntegration: Refresh button clicked');
                handleRefresh();
              }}
            ...
            <button
              onClick={() => {
                console.log('🖱️ ClasseraIntegration: Manual sync button clicked');
                triggerManualSync();
              }}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          ...
          <div>
            ...
            <div className="text-sm opacity-80">مؤسسات تعليمية متزامنة</div>
          </div>
          <div>
            <div className="text-2xl font-bold">
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={() => {
              console.log('🖱️ ClasseraIntegration: Toggle LTI config visibility clicked');
              console.log('🔄 ClasseraIntegration: Current showLTIConfig:', showLTIConfig, '-> New value:', !showLTIConfig);
              setShowLTIConfig(!showLTIConfig);
            }}
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={() => {
              console.log('🖱️ ClasseraIntegration: Toggle JWKS visibility clicked');
              console.log('🔄 ClasseraIntegration: Current showJWKS:', showJWKS, '-> New value:', !showJWKS);
              setShowJWKS(!showJWKS);
            }}
            className="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors"
          >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 text-sm">
                      {batch.schools_count} مؤسسة تعليمية، {batch.users_count} مستخدم
                    </p>
                  </div>
                </div>
