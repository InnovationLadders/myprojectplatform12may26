  ...
  const handleClick = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (err?.code === 'auth/popup-closed-by-user') {
        ...
        setError(err?.message || 'فشل تسجيل الدخول بحساب جوجل. يرجى المحاولة مرة أخرى.');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="space-y-3">
      <motion.button
        ...
        onClick={handleClick}
        disabled={isLoading}
        className={`w-full bg-white border border-gray-300 text-gray-700 py-3 rounded-xl font-medium hover:bg-gray-50 hover:shadow-md transition-all duration-200 flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
      >
        {isLoading ? (
          <>
          ...
          <>
            <GoogleIcon />
            <span>{label || t('auth.loginWithGoogle')}</span>
          </>
        )}
      </motion.button>
