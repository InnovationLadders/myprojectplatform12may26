...
import { createProjectTask } from '../../lib/firebase';
...
export const AddTaskModal: React.FC<AddTaskModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        onClose();
      }, 500); // Small delay to ensure the task is saved before closing
    } catch (err) {
      console.error('Error creating task:', err);
      setError(err instanceof Error ? err.message : 'حدث خطأ في إنشاء المهمة');
    } finally {
