...
export const IdeaFormModal: React.FC<IdeaFormModalProps> = ({ 
  ...
  useEffect(() => {
    if (editingIdea) {
      // Extract values from MultiLang format for editing
      const objectives = getArrayValue(editingIdea.objectives);
      const materials = getArrayValue(editingIdea.materials);
      const steps = getArrayValue(editingIdea.steps);
      const tags = getArrayValue(editingIdea.tags);

      setFormData({
        ...editingIdea,
        // Convert MultiLangText to plain strings
        title: getTextValue(editingIdea.title),
        description: getTextValue(editingIdea.description),
        duration: getTextValue(editingIdea.duration),
        subject: getTextValue(editingIdea.subject),
        // Convert MultiLangArray to plain arrays
