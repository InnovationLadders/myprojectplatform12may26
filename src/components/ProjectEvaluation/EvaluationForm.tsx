...
export const EvaluationForm: React.FC<EvaluationFormProps> = ({ projectId, projectTitle, onSaved }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await saveEvaluation(user?.id, user?.name, projectTitle);
      setSaveSuccess(true);
  ...
  const getCriterionIcon = (name: string) => {
    const criterionKey = name.toLowerCase().replace(/\s+/g, '');
    switch (criterionKey) {
      case 'نسبةالإنجاز':
      case 'completionrate':
      ...
      case 'التواصلوالتعاون':
      case 'communicationandcooperation':
      case 'communication&cooperation':
        return <Users className="w-5 h-5" />;
      case 'الإبداعوالابتكار':
      case 'creativityandinnovation':
      case 'creativity&innovation':
        return <Lightbulb className="w-5 h-5" />;
      case 'جودةالعرضوالتوثيق':
      case 'presentationanddocumentation':
      case 'presentationanddocumentationquality':
      case 'presentation&documentationquality':
    ...
    return (
      <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-red-700">
        <div className="flex items-center gap-2 mb-2">
          ...
          <p className="font-medium">{t('projectEvaluation.errorOccurred')}</p>
        </div>
        <p>{error}</p>
      </div>
    );
  ...
  if (!currentEvaluation) {
    return (
      <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-yellow-700">
        <p>{t('projectEvaluation.cannotLoadForm')}</p>
      </div>
    );
  }
    ...
    >
      ...
      {saveSuccess && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-green-700 mb-6">
          <div className="flex items-center gap-2">
            ...
            <p>{t('projectEvaluation.savedSuccessfully')}</p>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="space-y-6 mb-8">
          <h4 className="font-semibold text-gray-800 mb-4">{t('projectEvaluation.evaluationCriteria')}</h4>
            ...
            <div key={index} className="border border-gray-200 rounded-xl p-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={t('projectEvaluation.commentsPlaceholder')}
                />
              </div>
            </div>
          ))}
        ...
        <div className="mb-6">
          ...
          <textarea
            ...
            placeholder={t('projectEvaluation.generalCommentPlaceholder')}
          />
        </div>
          ...
          >
            {isSaving ? (
              <>
                ...
                {t('projectEvaluation.saving')}
              </>
            ) : (
              <>
                <Save className="w-5 h-5" />
                {t('projectEvaluation.saveEvaluation')}
              </>
            )}
          </button>
        </div>
