...
export const CreateProject: React.FC = () => {
  ...
  const { getProjectIdeaById } = useProjectIdeas();
  ...
  const [formData, setFormData] = useState({
    title: '',
    description: '',
    problemSolved: '', // Added the new field
    category: '',
    subject: '',
    difficulty: '',
    duration: '',
    ...
    if (ideaId) {
      const fetchProjectIdea = async () => {
        try {
          ...
          const idea = await getProjectIdeaById(ideaId);
          
          if (idea) {
            setUsingIdea(idea);

            // Pre-populate form with idea data using helper functions
            const objectives = getArrayValue(idea.objectives);
            const materials = getArrayValue(idea.materials);
            const steps = getArrayValue(idea.steps);

            setFormData(prev => ({
              ...prev,
              title: getTextValue(idea.title) || '',
              description: getTextValue(idea.description) || '',
              category: idea.category || '',
              subject: getTextValue(idea.subject) || '',
          ...
          setIdeaError('حدث خطأ في تحميل فكرة المشروع');
        } finally {
  ...
  const handleStudentToggle = (studentId: string) => {
    setSelectedStudentIds(prev => {
      if (prev.includes(studentId)) {
        ...
        if (currentStudents.length + prev.length >= formData.maxStudents) {
          return prev;
        }
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Ensure the current student is always included in their own project
      let finalSelectedStudentIds = [...selectedStudentIds];
      if (user?.role === 'student' && user?.id && !finalSelectedStudentIds.includes(user.id)) {
        finalSelectedStudentIds = [user.id, ...selectedStudentIds];
      }
      ...
      alert(t('createProject.errors.creationFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  // Get available grades (only grades with students)
  const availableGrades = getAvailableGrades(students);
  ...
  // Then filter by grade
  const filteredStudents = filterStudentsByGrade(searchFilteredStudents, selectedGrade);

  // Get current students (for UI display)
  ...
  return (
    <div className="space-y-6">
      ...
      >
        {/* Loading state for idea fetching */}
        {ideaLoading && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6 flex items-center gap-3">
            <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-blue-500"></div>
            <div>
              <h4 className="font-medium text-blue-800">جاري تحميل فكرة المشروع...</h4>
              <p className="text-blue-700 text-sm">يتم تحضير البيانات لك</p>
            </div>
          </div>
        )}

        {/* Error state for idea fetching */}
        {ideaError && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-6 flex items-start gap-3">
            <AlertTriangle className="w-5 h-5 text-red-500 mt-0.5 flex-shrink-0" />
            <div>
              <h4 className="font-medium text-red-800">خطأ في تحميل فكرة المشروع</h4>
              <p className="text-red-700 text-sm">{ideaError}</p>
            </div>
          </div>
        )}

        {/* Success state for idea loading */}
        {usingIdea && !ideaLoading && !ideaError && (
          <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-6 flex items-start gap-3">
            <CheckCircle className="w-5 h-5 text-green-500 mt-0.5 flex-shrink-0" />
            <div>
              <h4 className="font-medium text-green-800">تم تحميل فكرة المشروع بنجاح</h4>
              <p className="text-green-700 text-sm">
                تم تحميل بيانات فكرة المشروع "{getTextValue(usingIdea.title)}" وملء النموذج. يمكنك تعديل أي من هذه البيانات حسب احتياجاتك.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div className="flex-1">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">
              {usingIdea ? `إنشاء مشروع من فكرة: ${getTextValue(usingIdea.title)}` : t('createProject.title')}
            </h1>
            <p className="text-gray-600">
              {usingIdea ? 'تم تحميل بيانات فكرة المشروع. يمكنك تعديل أي من هذه البيانات.' : t('createProject.subtitle')}
            </p>
          </div>
        </div>
      ...
      >
        ...
        {currentStep === 1 && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600">{t('createProject.startWithBasicInfo')}</p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {t('createProject.form.problemSolved')}
                </label>
                <textarea
                  ...
                  rows={3}
                  className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder={t('createProject.form.problemSolvedPlaceholder')}
                />
              </div>
        ...
        {currentStep === 2 && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600">{t('createProject.defineObjectives')}</p>
            </div>
        ...
        {currentStep === 3 && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600">{t('createProject.defineMaterialsSteps')}</p>
            </div>
        ...
        {currentStep === 4 && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600">{t('createProject.setDeadlineStatus')}</p>
            </div>
            ...
            <div className="bg-gray-50 rounded-xl p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-600">نسبة الإنجاز الأولية:</span>
        ...
        {currentStep === 5 && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              <h2 className="text-2xl font-bold text-gray-800 mb-2">{t('createProject.steps.addStudents')}</h2>
              <p className="text-gray-600">{t('createProject.chooseParticipatingStudents')}</p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="relative">
                ...
                <select
                  ...
                  onChange={(e) => setSelectedGrade(e.target.value === 'all' ? 'all' : e.target.value)}
                  className="w-full pr-12 pl-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white appearance-none cursor-pointer"
                >
                  <option value="all">جميع المراحل/المستويات ({students.length})</option>
                  {availableGrades.map((grade) => (
                    <option key={grade.normalized} value={grade.normalized}>
                      {grade.displayName} ({grade.count})
                    </option>
                  ))}
                </select>
              ...
              <div className="bg-green-50 border border-green-200 rounded-xl p-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-green-800 text-sm font-medium">
                    يتم عرض {filteredStudents.length} طالب من {availableGrades.find(g => g.normalized === selectedGrade)?.displayName}
                  </span>
                </div>
                <button
                ...
                filteredStudents.map((student) => (
                  ...
                  >
                    <input
                      ...
                      disabled={!selectedStudentIds.includes(student.id) && currentStudents.length + selectedStudentIds.length >= formData.maxStudents}
                      className="text-blue-600"
                    />
                      ...
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        {student.grade && (
                          <span className="px-2 py-1 bg-purple-100 text-purple-800 text-xs rounded-full flex items-center gap-1">
                            ...
                            {classifyGrade(student.grade).displayName}
                          </span>
                        )}
                      </div>
            ...
            <div className="bg-gray-50 rounded-xl p-4">
              ...
              <ul className="text-sm text-gray-600 space-y-1">
                ...
                <li>• {t('createProject.canModifyLater')}</li>
          ...
          <div className="flex gap-3">
            ...
            {currentStep < 5 ? (
              ...
              >
                ...
                {loading ? t('createProject.creating') : t('createProject.createProject')}
              </button>
            )}
          </div>
        </div>
