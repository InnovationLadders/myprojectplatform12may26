  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div
        ...
        className="bg-gradient-to-r from-emerald-600 to-teal-600 rounded-xl md:rounded-2xl p-4 md:p-6 text-white"
      >
        <div className="flex items-center gap-2 md:gap-4 mb-4">
          ...
          <div>
            <h1 className="text-xl md:text-2xl lg:text-3xl font-bold leading-tight">{t('resources.title')}</h1>
            <p className="opacity-90 text-sm md:text-base mt-0.5">{t('resources.subtitle')}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 text-center">
          <div className="bg-white bg-opacity-10 rounded-lg p-2 md:p-3">
            ...
            <div className="text-xs md:text-sm opacity-80">{t('resources.stats.learningResource')}</div>
          </div>
          <div className="bg-white bg-opacity-10 rounded-lg p-2 md:p-3">
            <div className="text-xl md:text-2xl font-bold">{resourceTypes.length - 1}</div>
            <div className="text-xs md:text-sm opacity-80">{t('resources.stats.contentType')}</div>
          </div>
          <div className="bg-white bg-opacity-10 rounded-lg p-2 md:p-3">
            <div className="text-xl md:text-2xl font-bold">{categories.length - 1}</div>
            <div className="text-xs md:text-sm opacity-80">{t('resources.stats.specialtyCategory')}</div>
          </div>
          <div className="bg-white bg-opacity-10 rounded-lg p-2 md:p-3">
            <div className="text-xl md:text-2xl font-bold">100%</div>
            <div className="text-xs md:text-sm opacity-80">{t('resources.stats.free')}</div>
          </div>
        </div>
      </motion.div>
      ...
      >
        <div className="flex flex-col lg:flex-row gap-3 md:gap-4 mb-4 md:mb-6">
          ...
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pr-10 md:pr-12 pl-3 md:pl-4 py-2.5 md:py-3 text-sm md:text-base border border-gray-300 rounded-lg md:rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
            />
          </div>
        </div>
        ...
        <div className="mb-4 md:mb-6">
          <h3 className="font-medium text-gray-700 mb-3 text-sm md:text-base">{t('resources.filters.contentType')}</h3>
          <div className="flex flex-wrap gap-2">
            {resourceTypes.map((type) => (
              <button
                key={type.id}
                onClick={() => setSelectedType(type.id)}
                className={`flex items-center gap-1.5 md:gap-2 px-3 md:px-4 py-2 rounded-lg md:rounded-xl transition-all text-xs md:text-sm whitespace-nowrap ${
                  selectedType === type.id
                    ? 'bg-emerald-500 text-white shadow-lg'
                    : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
              >
                <type.icon className="w-3.5 h-3.5 md:w-4 md:h-4" />
                {type.name}
              </button>
            ))}
          </div>
        ...
        {/* Category Filters */}
        <div>
          <h3 className="font-medium text-gray-700 mb-3 text-sm md:text-base">{t('resources.filters.category')}</h3>
          <div className="flex flex-wrap gap-2">
              ...
              <div className="p-6">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <button className="flex-1 sm:flex-none p-2.5 rounded-lg bg-gray-100 text-gray-600 hover:bg-blue-100 hover:text-blue-600 transition-colors">
                      <Share2 className="w-4 h-4 mx-auto" />
                    </button>
                    {(resource.type === 'template' || resource.downloadUrl) && (
                      <button className="flex-1 sm:flex-none p-2.5 rounded-lg bg-gray-100 text-gray-600 hover:bg-green-100 hover:text-green-600 transition-colors">
                  ...
                  {resource.type === 'video' ? (
                    <a
                      ...
                      className="w-full sm:w-auto px-3 md:px-4 py-2.5 md:py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 transition-colors text-xs md:text-sm font-medium flex items-center justify-center gap-2 whitespace-nowrap"
                    >
      ...
      {filteredResources.length === 0 && (
        ...
        >
          ...
          <button
            ...
            className="w-full sm:w-auto px-4 md:px-6 py-2.5 md:py-2 bg-emerald-500 text-white rounded-lg md:rounded-xl hover:bg-emerald-600 transition-colors text-sm md:text-base"
          >
