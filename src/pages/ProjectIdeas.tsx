...
import { matchesSearchTerm, getTextValue, getArrayValue } from '../utils/multiLangHelper';
...
export const ProjectIdeas: React.FC = () => {
  ...
  const filteredProjects = projectIdeas
    .filter(project => {
      ...
      const matchesSearch = matchesSearchTerm(project.title, searchTerm) ||
                           matchesSearchTerm(project.description, searchTerm);
...
## ${t('projectIdeas.modal.description')}
${idea.description}
...
## ${t('projectIdeas.modal.expectedDuration')}
${idea.duration || t('projectIdeas.modal.notSpecified')}

## ${t('projectIdeas.modal.subject')}
${idea.subject || t('projectIdeas.modal.notSpecified')}

## ${t('projectIdeas.modal.objectives')}
${idea.objectives?.map((obj: string) => `- ${obj}`).join('\n') || t('projectIdeas.modal.notSpecified')}

## ${t('projectIdeas.modal.materials')}
${idea.materials?.map((mat: string) => `- ${mat}`).join('\n') || t('projectIdeas.modal.notSpecified')}

## ${t('projectIdeas.modal.steps')}
${idea.steps?.map((step: string, index: number) => `${index + 1}. ${step}`).join('\n') || t('projectIdeas.modal.notSpecified')}

## ${t('common.tags')}
${idea.tags?.map((tag: string) => `#${tag}`).join(' ') || t('projectIdeas.modal.notSpecified')}
        `;
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div
        ...
        className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-4 md:p-6 text-white"
      >
        <div className="flex items-center gap-3 md:gap-4 mb-4">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold">{t('projectIdeas.title')}</h1>
            <p className="opacity-90 text-sm md:text-base">{t('projectIdeas.subtitle')}</p>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <button
          ...
          className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 md:px-6 py-2.5 md:py-2 bg-blue-500 text-white rounded-lg md:rounded-xl hover:bg-blue-600 transition-colors text-sm md:text-base whitespace-nowrap"
        >
          ...
          >
            ...
            <div className="p-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm text-gray-600">{getTextValue(project.subject) || getCategoryText(project.category)}</span>
              </div>
              ...
              <p className="text-gray-600 mb-4 line-clamp-3 leading-relaxed">
                {getTextValue(project.description)}
              </p>

              <div className="flex items-center justify-between text-sm text-gray-500 mb-4">
                <div className="flex items-center gap-1">
                  <Clock className="w-4 h-4" />
                  {getTextValue(project.duration) || t('resources.defaultDuration')}
                </div>
                <div className="flex items-center gap-1">
              ...
              <div className="flex gap-2">
                <button
                  className="flex-1 bg-blue-500 text-white py-2.5 px-4 rounded-lg md:rounded-xl hover:bg-blue-600 transition-colors font-medium text-sm md:text-base"
                  onClick={() => handleViewIdea(project.id)}
                >
                  {t('projectIdeas.viewDetails')}
                </button>
                <button
                  className="px-3 md:px-4 py-2.5 border border-gray-300 rounded-lg md:rounded-xl hover:bg-gray-50 transition-colors"
      ...
      {filteredProjects.length === 0 && (
        ...
        >
          ...
          <h3 className="text-lg md:text-xl font-semibold text-gray-800 mb-2">{t('projectIdeas.noResults')}</h3>
          <p className="text-sm md:text-base text-gray-600 mb-4">{t('projectIdeas.tryChangingFilters')}</p>
          <button
            onClick={() => {
            ...
            className="w-full sm:w-auto px-4 md:px-6 py-2.5 md:py-2 bg-blue-500 text-white rounded-lg md:rounded-xl hover:bg-blue-600 transition-colors text-sm md:text-base"
          >
              ...
              <div className="relative h-64 bg-gradient-to-r from-blue-600 to-purple-600">
                {selectedIdea.image ? (
                  <img
                    ...
                    alt={getTextValue(selectedIdea.title)}
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-6">
                  ...
                  <h2 className="text-2xl font-bold text-white">{getTextValue(selectedIdea.title)}</h2>
                </div>
              </div>
              ...
              <div className="p-6">
                <div className="grid md:grid-cols-3 gap-6 mb-6">
                  <div className="md:col-span-2">
                    <h3 className="font-semibold text-gray-800 mb-2">{t('projectIdeas.modal.description')}</h3>
                    <p className="text-gray-600 leading-relaxed mb-4">{getTextValue(selectedIdea.description)}</p>
                    
                    {/* Tags */}
                    {getArrayValue(selectedIdea.tags).length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-4">
                        {getArrayValue(selectedIdea.tags).map((tag: string, idx: number) => (
                  ...
                  <div>
                    <div className="bg-gray-50 rounded-xl p-4">
                      <h3 className="font-semibold text-gray-800 mb-3">{t('projectIdeas.modal.projectInfo')}</h3>

                      <div className="space-y-3">
                        <div>
                          <p className="text-sm text-gray-500">{t('projectIdeas.modal.expectedDuration')}</p>
                          <p className="font-medium">{getTextValue(selectedIdea.duration) || t('projectIdeas.modal.notSpecified')}</p>
                        </div>

                        <div>
                          <p className="text-sm text-gray-500">{t('projectIdeas.modal.subject')}</p>
                          <p className="font-medium">{getTextValue(selectedIdea.subject) || t('projectIdeas.modal.notSpecified')}</p>
                        </div>

                        <div>
                          <p className="text-sm text-gray-500">{t('projectIdeas.modal.rating')}</p>
                          <div className="flex items-center gap-1">
                            <Star className="w-4 h-4 text-yellow-400 fill-current" />
                        ...
                        <div>
                          <p className="text-sm text-gray-500">{t('projectIdeas.modal.views')}</p>
                          <p className="font-medium">{selectedIdea.views}</p>
                        </div>

                        <div>
                          <p className="text-sm text-gray-500">{t('projectIdeas.modal.downloads')}</p>
                ...
                {/* Objectives */}
                {getArrayValue(selectedIdea.objectives).length > 0 && (
                  <div className="mb-6">
                    <h3 className="font-semibold text-gray-800 mb-3">{t('projectIdeas.modal.objectives')}</h3>
                    <div className="space-y-2">
                      {getArrayValue(selectedIdea.objectives).map((objective: string, idx: number) => (
                ...
                {/* Materials */}
                {getArrayValue(selectedIdea.materials).length > 0 && (
                  <div className="mb-6">
                    <h3 className="font-semibold text-gray-800 mb-3">{t('projectIdeas.modal.materials')}</h3>
                    <div className="grid md:grid-cols-2 gap-2">
                      {getArrayValue(selectedIdea.materials).map((material: string, idx: number) => (
                ...
                {/* Steps */}
                {getArrayValue(selectedIdea.steps).length > 0 && (
                  <div className="mb-6">
                    <h3 className="font-semibold text-gray-800 mb-3">{t('projectIdeas.modal.steps')}</h3>
                    <div className="space-y-3">
                      {getArrayValue(selectedIdea.steps).map((step: string, idx: number) => (
                        <div key={idx} className="flex items-start gap-3">
                ...
                <div className="flex flex-col sm:flex-row justify-between gap-4 pt-4 border-t border-gray-200">
                  <div className="flex items-center gap-3 md:gap-4 text-sm md:text-base">
                    <div className="flex items-center gap-1 text-gray-600">
                      ...
                      <span>{selectedIdea.views} {t('projectIdeas.modal.view')}</span>
                    </div>
                    <div className="flex items-center gap-1 text-gray-600">
                      <Download className="w-4 h-4 md:w-5 md:h-5" />
                      <span>{selectedIdea.downloads} {t('projectIdeas.modal.download')}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-2 w-full sm:w-auto">
                    <button
                      ...
                      className="w-full sm:w-auto px-4 md:px-6 py-2.5 md:py-2 bg-blue-500 text-white rounded-lg md:rounded-xl hover:bg-blue-600 transition-colors flex items-center justify-center gap-2 text-sm md:text-base whitespace-nowrap"
                    >
                      <Download className="w-4 h-4 md:w-5 md:h-5" />
                      {t('projectIdeas.modal.download')}
                    </button>
                    <button
                      onClick={() => handleUseIdea(selectedIdea.id)}
                      className="w-full sm:w-auto px-4 md:px-6 py-2.5 md:py-2 bg-green-500 text-white rounded-lg md:rounded-xl hover:bg-green-600 transition-colors flex items-center justify-center gap-2 text-sm md:text-base whitespace-nowrap"
                    >
                      <PlayCircle className="w-4 h-4 md:w-5 md:h-5" />
                      {t('projectIdeas.modal.useIdea')}
                    </button>
                    <button
                      onClick={() => setSelectedIdea(null)}
                      className="w-full sm:w-auto px-4 md:px-6 py-2.5 md:py-2 border border-gray-300 rounded-lg md:rounded-xl hover:bg-gray-50 transition-colors text-sm md:text-base whitespace-nowrap"
                    >
                      {t('common.close')}
                    </button>
                  </div>
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <button
              onClick={() => setShowIdeaFormModal(true)}
              className="w-full sm:w-auto px-4 md:px-6 py-2.5 md:py-2 bg-blue-500 text-white rounded-lg md:rounded-xl hover:bg-blue-600 transition-colors flex items-center justify-center gap-2 mx-auto text-sm md:text-base"
            >
              <Plus className="w-4 h-4" />
              اقتراح فكرة جديدة
            </button>
