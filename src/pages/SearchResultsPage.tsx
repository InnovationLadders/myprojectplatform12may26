...
import { getTextValue, matchesSearchTerm, arrayContainsSearchTerm } from '../utils/multiLangHelper';
...
export const SearchResultsPage: React.FC = () => {
  ...
  const filteredIdeas = projectIdeas.filter(idea =>
    matchesSearchTerm(idea.title, searchQuery) ||
    matchesSearchTerm(idea.description, searchQuery) ||
    arrayContainsSearchTerm(idea.tags, searchQuery)
  );
            ...
            <div className="space-y-4">
              {results.ideas.slice(0, 5).map((idea) => (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <h3 className="text-lg font-bold text-gray-800 mb-2">{getTextValue(idea.title)}</h3>
                      <p className="text-gray-600 line-clamp-2">{getTextValue(idea.description)}</p>
