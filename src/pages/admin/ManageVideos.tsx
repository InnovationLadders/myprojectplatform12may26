...
import { isMockVideoUrl } from '../../utils/videoValidation';
...
const ManageVideos: React.FC = () => {
  ...
  // Count mock videos
  const mockVideosCount = videos.filter(video =>
    isMockVideoUrl(video.videoUrl) ||
    isMockVideoUrl(video.watchUrl) ||
    isMockVideoUrl(video.embedUrl) ||
    isMockVideoUrl(video.contentUrl)
  ).length;
      ...
      {mockVideosCount > 0 && (
        ...
        >
          ...
          <div className="flex-1">
            <h3 className="font-bold text-red-800 mb-1">تحذير: فيديوهات بروابط وهمية!</h3>
            <p className="text-red-700 mb-2">
              يوجد {mockVideosCount} فيديو يحتوي على روابط وهمية. هذه الروابط لن تعمل للمستخدمين!
            </p>
            <p className="text-red-600 text-sm mb-3">
              الرجاء تحديث هذه الفيديوهات بروابط حقيقية من YouTube أو حذفها.
            </p>
            <button
              onClick={handleCleanupDuplicates}
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredVideos.map((video) => {
                const isMockVideo = isMockVideoUrl(video.videoUrl) ||
                                   isMockVideoUrl(video.watchUrl) ||
                                   isMockVideoUrl(video.embedUrl) ||
                                   isMockVideoUrl(video.contentUrl);

                return (
                  <tr key={video.id} className={`hover:bg-gray-50 ${isMockVideo ? 'bg-red-50' : ''}`}>
