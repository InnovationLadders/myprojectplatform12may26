...
export const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    console.log('✅ AdminDashboard: Component mounted successfully');
    console.log('📊 AdminDashboard: Initial activeTab:', 'overview');
  ...
  const dashboardTabs = [
    ...
    { id: 'ai-assistant', name: 'المساعد الذكي', icon: BarChart3 },
  ...
  const quickActions = [
    {
      title: 'استعادة بيانات المستخدمين',
      description: 'فحص وإصلاح بيانات المستخدمين المفقودة أو التالفة',
      icon: Database,
    ...
    {
      title: 'إصلاح بيانات ريادة الأعمال',
      description: 'تشخيص وإصلاح مشاكل school_id في مشاريع ريادة الأعمال',
      icon: Wrench,
      link: '/admin/fix-entrepreneurship-data',
      color: 'from-orange-500 to-red-500',
      urgent: true
    },
    {
      title: 'تخصيص المؤسسات التعليمية',
      description: 'إدارة الدومين الفرعي والشعار لكل مؤسسة',
      icon: Building,
      link: '/admin/school-customization',
  ...
  console.log('🎨 AdminDashboard: Rendering component with activeTab:', activeTab);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          ...
          <nav className="flex overflow-x-auto">
            {dashboardTabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => {
                  console.log('🖱️ AdminDashboard: Tab clicked:', tab.id, tab.name);
                  setActiveTab(tab.id);
                }}
        ...
        <div className="p-6">
          ...
          {/* School Customization Tab */}
          {activeTab === 'school-customization' && (
            <div className="text-center py-8">
              <Building className="w-16 h-16 text-gray-400 mx-auto mb-4" />
              <h3 className="text-xl font-semibold text-gray-800 mb-2">تخصيص المؤسسات التعليمية</h3>
              <p className="text-gray-600 mb-4">إدارة الدومين الفرعي والشعار لكل مؤسسة تعليمية</p>
              <Link
                to="/admin/school-customization"
                className="px-6 py-2 bg-indigo-500 text-white rounded-xl hover:bg-indigo-600 transition-colors"
              >
                إدارة التخصيصات
              </Link>
          ...
          {activeTab === 'ai-assistant' && (
            <div className="text-center py-8">
              ...
              <h3 className="text-xl font-semibold text-gray-800 mb-2">إدارة المساعد الذكي</h3>
              <p className="text-gray-600 mb-4">إدارة وتكوين خدمة المساعد الذكي</p>
              <Link
                to="/admin/manage-ai-assistant"
                className="px-6 py-2 bg-indigo-500 text-white rounded-xl hover:bg-indigo-600 transition-colors"
              >
                إدارة المساعد الذكي
              </Link>
