...
import { Mail, Lock, Eye, EyeOff, TriangleAlert as AlertTriangle } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  ...
  const { schoolName, schoolId, logoUrl } = useSchoolBranding();
  const currentSubdomain = getSubdomain();

  useEffect(() => {
    if (currentSubdomain && schoolName) {
      setSubdomainWarning(
        `تحذير: أنت تحاول تسجيل الدخول من رابط ${schoolName}. تأكد من أنك تنتمي لهذه المؤسسة التعليمية.`
      );
    }
  }, [currentSubdomain, schoolName]);
