...
export const CheckoutPage: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.fullName || !formData.email || !formData.phone || !formData.address || !formData.city) {
      setFormError(t('checkout.errors.fillRequired'));
      return false;
    }

    if (!formData.agreeToTerms) {
      setFormError(t('checkout.errors.agreeTerms'));
      return false;
    }
    ...
    if (!emailRegex.test(formData.email)) {
      setFormError(t('checkout.errors.invalidEmail'));
      return false;
    }
    ...
    if (!phoneRegex.test(formData.phone.replace(/\D/g, ''))) {
      setFormError(t('checkout.errors.invalidPhone'));
      return false;
    }
    ...
    if (formData.paymentMethod === 'credit-card') {
      if (!formData.cardNumber || !formData.cardName || !formData.cardExpiry || !formData.cardCvv) {
        setFormError(t('checkout.errors.fillCardDetails'));
        return false;
      }
      ...
      if (!cardNumberRegex.test(formData.cardNumber.replace(/\D/g, ''))) {
        setFormError(t('checkout.errors.invalidCardNumber'));
        return false;
      }
      ...
      if (!expiryRegex.test(formData.cardExpiry)) {
        setFormError(t('checkout.errors.invalidExpiry'));
        return false;
      }
      ...
      if (!cvvRegex.test(formData.cardCvv)) {
        setFormError(t('checkout.errors.invalidCvv'));
        return false;
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(t('checkout.errors.processingError'));
    } finally {
